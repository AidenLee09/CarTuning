...
The app is wired for Vertex AI through the Vercel Serverless Function at `api/analyze.js`. Add the variables in `.env.example` to Vercel as server-side environment variables to call Gemini 3 Flash Preview through the `v1beta1` endpoint. Without credentials, Apex Agent generates a local race-engineer demo report so the dashboard remains testable.
